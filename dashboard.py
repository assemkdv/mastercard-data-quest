...
# ── Precompute hourly/DOW aggregates ──────────────────────────
...
MCC_NAMES = {
    ...
    "5065":"Electronics","5085":"Industrial Equipment",
}
...
# Score distribution for consumers
...
KPI_STYLE = {**CARD_STYLE, "textAlign": "center", "padding": "24px 16px"}
...
app.layout = html.Div(style={"background":C_BG,"minHeight":"100vh","padding":"24px",
                              "fontFamily":"Inter, sans-serif","color":C_TEXT}, children=[
    ...
    # ── Tab navigation ───────────────────────────────────────
    dcc.Tabs(id="tabs", value="overview", style={"marginBottom":"20px"},
             children=[
...
def render_tab(tab):
    ...
    if tab == "overview":
        ...
        fig_score.add_trace(go.Histogram(
            x=consumer_scores, nbinsx=80, name="Consumer Scores",
        ...
        fig_score.update_layout(**PLOTLY_LAYOUT,
            ...
            xaxis_title="Business Score (0 = pure consumer, 1 = business-like)",
            yaxis_title="Number of Cards")

        # Scatter: txn_count vs avg_amount coloured by segment
        sample = feat.sample(min(5000, len(feat)), random_state=42)
        fig_scatter = px.scatter(
            sample, x="txn_count", y="avg_amount",
            color="segment",
        ...
        fig_monthly.add_trace(go.Scatter(
            x=[month_names[m] for m in monthly_biz.index],
            y=monthly_biz.values, name="Business Cards",
            line=dict(color=C_BIZ, width=2), mode="lines+markers"))
        fig_monthly.add_trace(go.Scatter(
            x=[month_names[m] for m in monthly_con.index],
            y=monthly_con.values, name="Consumer Cards",
            line=dict(color=C_CON, width=2), mode="lines+markers"))
        ...
        return html.Div([
            kpi_row,
            html.Div([
                html.Div(dcc.Graph(figure=fig_score), style={**CARD_STYLE,"flex":"1.5"}),
                html.Div(dcc.Graph(figure=fig_monthly), style={**CARD_STYLE,"flex":"1"}),
    ...
    # ════════════════════════════════════════════════════════
        ...
        # Hour of day
        fig_hour = go.Figure()
        fig_hour.add_trace(go.Scatter(x=hour_biz["hour"], y=hour_biz["pct"],
            name="Business", line=dict(color=C_BIZ,width=2), fill="tozeroy",
            fillcolor="rgba(244,147,76,0.15)"))
        fig_hour.add_trace(go.Scatter(x=hour_con["hour"], y=hour_con["pct"],
            name="Consumer", line=dict(color=C_CON,width=2), fill="tozeroy",
            fillcolor="rgba(76,123,244,0.15)"))
        ...
        fig_hour.update_layout(**PLOTLY_LAYOUT, title="Transaction Activity by Hour of Day",
            xaxis_title="Hour", yaxis_title="% of Transactions",
            xaxis=dict(tickvals=list(range(0,24)),gridcolor="#2A2D3A",zerolinecolor="#2A2D3A"))
        ...
        fig_dow.update_layout(**PLOTLY_LAYOUT, title="Transaction Activity by Day of Week",
        ...
        fig_mcc.update_layout(**PLOTLY_LAYOUT, showlegend=False,
            title="Top Merchant Categories by Segment", height=380)

        # Radar chart of avg feature values
        ...
        radar_labels = ["Online","Recurring","B2B MCC","Weekday","Large Txn","Tokenized","Foreign Merchant"]
        fig_radar = go.Figure()
        for vals, name, color in [(biz_vals,"Business",C_BIZ),(con_vals,"Consumer",C_CON),(hid_vals,"Hidden Entrep.",C_HIDDEN)]:
        ...
        fig_radar.update_layout(**PLOTLY_LAYOUT, polar=dict(
            bgcolor=C_CARD,
            radialaxis=dict(visible=True, range=[0,1], gridcolor="#2A2D3A",color=C_MUTED),
            angularaxis=dict(gridcolor="#2A2D3A",color=C_TEXT)),
            title="Behavioural Profile Radar")

        return html.Div([
            html.Div([
                html.Div(dcc.Graph(figure=fig_hour), style={**CARD_STYLE,"flex":"1"}),
                html.Div(dcc.Graph(figure=fig_dow),  style={**CARD_STYLE,"flex":"1"}),
            ], style={"display":"flex","gap":"16px"}),
            html.Div([
                html.Div(dcc.Graph(figure=fig_mcc),   style={**CARD_STYLE,"flex":"2"}),
                html.Div(dcc.Graph(figure=fig_radar),  style={**CARD_STYLE,"flex":"1"}),
            ], style={"display":"flex","gap":"16px"}),
        ])
    ...
    # ════════════════════════════════════════════════════════
        ...
        fig_thresh.add_vline(x=THRESHOLD, line_dash="dash", line_color="red",
                             annotation_text=f"Threshold={THRESHOLD}", annotation_font_color="red")
        fig_thresh.update_layout(**PLOTLY_LAYOUT,
        ...
        fig_corr.update_layout(**PLOTLY_LAYOUT, title="Feature Correlation (top features + label)", height=420)

        return html.Div([
            html.Div([
                html.Div(dcc.Graph(figure=fig_fi),     style={**CARD_STYLE,"flex":"1"}),
                html.Div(dcc.Graph(figure=fig_roc),    style={**CARD_STYLE,"flex":"1"}),
            ], style={"display":"flex","gap":"16px"}),
            html.Div([
                html.Div(dcc.Graph(figure=fig_thresh), style={**CARD_STYLE,"flex":"1"}),
                html.Div(dcc.Graph(figure=fig_corr),   style={**CARD_STYLE,"flex":"1"}),
            ], style={"display":"flex","gap":"16px"}),
        ])
    ...
    # ════════════════════════════════════════════════════════
        ...
        fig_top.update_layout(**PLOTLY_LAYOUT, yaxis={"categoryorder":"total ascending"},
                              coloraxis_showscale=False, height=500)

        return html.Div([
            html.Div(dcc.Graph(figure=fig_top), style=CARD_STYLE),
            html.Div([
                ...
                dash_table.DataTable(
                    ...
                    style_cell={"background":C_CARD,"color":C_TEXT,"border":"1px solid #2A2D3A",
                                "fontFamily":"Inter, sans-serif","fontSize":"13px","padding":"8px 12px"},
                    style_header={"background":"#2A2D3A","fontWeight":"600","color":C_TEXT},
                    style_data_conditional=[
                        {"if":{"filter_query":"{Score} >= 0.41"},
                         "color":C_HIDDEN,"fontWeight":"600"}
                    ],
