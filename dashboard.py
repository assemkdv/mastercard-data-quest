...
def L(**kw):
    """Merge BASE layout with per-chart overrides, avoiding duplicate-key conflicts."""
...
# (feature_col, display_label, higher_means_business)
EXPLAIN_FEATURES = [
...
def get_top_signals(card_row, n=5):
    ...
    for feat_name, label, higher_is_biz in EXPLAIN_FEATURES:
        ...
        signals.append({
            "feature": label, "card_val": card_val,
...
def tier_label(score):
    """Return (label, color, description) for a given business score."""
    if score >= 0.75:
        return ("Direct Outreach", C_RED,
                "Priority conversion candidate — strong business signals across multiple features.")
    if score >= 0.50:
...
TAB_SELECTED = {
    **TAB_STYLE, "background": C_RED, "color": C_TEXT,
...
mc_logo = html.Div([
    ...
    html.Div(style={"width": "36px", "height": "36px", "borderRadius": "50%",
                    "background": C_YELLOW, "position": "absolute", "left": "22px",
                    "opacity": "0.9"}),
...
SYNTHETIC_WARNING = html.Div([
    ...
    "ROC-AUC = 1.000 is expected on this synthetic dataset — the generator creates very clean "
    "behavioural separation between card segments. In a real-world deployment AUC would typically "
    "be 0.75–0.90 and threshold tuning would matter significantly more.",
], style=WARN)
...
PERSONAS = [
    {
        ...
        "desc": "Runs e-commerce or drop-shipping. Heavy spend on Meta/Google Ads, logistics platforms "
                "and SaaS tools — entirely online, concentrated on weekdays.",
        "action": "Offer: Business card + merchant acquiring + working capital loan",
    },
    {
        "title": "International Freelancer", "icon": "💻", "color": C_ORANGE,
        "signals": "High foreign merchant ratio · GitHub/AWS/Stripe MCCs · Consulting fees",
        "desc": "Software developer or designer serving international clients. Pays for cloud tools "
                "and developer infrastructure; receives Stripe or Wise payouts.",
        "action": "Offer: Multi-currency business card + FX fee waiver",
    },
    {
        "title": "Traditional Merchant", "icon": "🏪", "color": C_YELLOW,
        "signals": "Higher POS ratio · Wholesale MCCs (5045, 5085) · Large transactions",
        "desc": "Brick-and-mortar shop owner buying inventory from wholesalers. Large individual "
                "transactions, consistent monthly spending, some POS activity.",
    ...
    },
    {
        ...
        "desc": "Operates a subscription service or platform. Recurring charges dominate — "
                "hosting, CRM, payment processors, marketing automation.",
    ...
    },
    {
        ...
        "desc": "Growing side business alongside regular employment. Mixed consumer and business "
                "patterns — early stage, signals are still forming.",
...
def render_tab(tab):
    ...
    if tab == "overview":
        ...
        fig_score = go.Figure(go.Histogram(
            x=consumer_scores, nbinsx=80, marker_color=C_RED, opacity=0.8))
            ...
            html.Div([
                html.Div([
                    html.Div([
                        ...
                        html.Span(p["title"], style={"fontWeight": "700",
                                                      "color": p["color"], "fontSize": "0.9rem"}),
    ...
    # ── BEHAVIOUR ────────────────────────────────────────────
        ...
        fig_hour.add_trace(go.Scatter(
            x=hour_biz["hour"], y=hour_biz["pct"], name="Business",
            line=dict(color=C_BIZ, width=2), fill="tozeroy",
            fillcolor="rgba(235,0,27,0.15)"))
        fig_hour.add_trace(go.Scatter(
            x=hour_con["hour"], y=hour_con["pct"], name="Consumer",
            line=dict(color=C_CON, width=2), fill="tozeroy",
            fillcolor="rgba(247,158,27,0.15)"))
        fig_hour.add_vrect(x0=9, x1=18, fillcolor="rgba(255,255,255,0.04)",
                           line_width=0, annotation_text="Business hours 9–18",
        ...
        fig_mcc = make_subplots(rows=1, cols=2, subplot_titles=[
            "Top 10 MCCs – Business Cards", "Top 10 MCCs – Consumer Cards"])
        fig_mcc.add_trace(go.Bar(x=mcc_biz["count"], y=mcc_biz["name"],
                                 orientation="h", marker_color=C_BIZ, name="Business"),
                          row=1, col=1)
        fig_mcc.add_trace(go.Bar(x=mcc_con["count"], y=mcc_con["name"],
                                 orientation="h", marker_color=C_CON, name="Consumer"),
                          row=1, col=2)
        ...
        radar_labels = ["Online", "Recurring", "B2B MCC", "Weekday",
                        "Large Txn", "Tokenized", "Foreign"]
        ...
        FILL_COLORS = {
            "#EB001B": "rgba(235,0,27,0.15)",
            "#F79E1B": "rgba(247,158,27,0.15)",
            "#FF5F00": "rgba(255,95,0,0.15)",
        }
        fig_radar = go.Figure()
        for vals, name, color in [
            (biz_vals, "Business", C_BIZ),
            (con_vals, "Consumer", C_CON),
        ...
        fig_radar.update_layout(**L(
            ...
            polar=dict(
                ...
                radialaxis=dict(visible=True, range=[0, 1],
                                gridcolor=C_GRID, color=C_MUTED),
    ...
    # ── MODEL ────────────────────────────────────────────────
        ...
        fig_roc.add_trace(go.Scatter(x=[0, 1], y=[0, 1],
                                     line=dict(color=C_MUTED, dash="dash"),
                                     showlegend=False))
        fig_roc.update_layout(**L(
            title="ROC Curve — Synthetic Dataset (AUC=1.000 expected)",
        ...
        fig_corr.update_layout(**L(
            title="Feature Correlation (top features + label)", height=420))
    ...
    # ── CANDIDATES ───────────────────────────────────────────
        ...
        tier_row = html.Div([
            ...
            html.Div([
                ...
                html.Div("Cross-sell: POS Acquiring · Working Capital Loans · Payroll · Trade Finance",
        ...
        # Prepare table (add tier before rounding score)
        ...
        table_df.columns = ["Card Number", "Score", "Tier", "Txn Count",
                             "Spend (M KZT)", "Online", "Recurring",
                             "B2B MCC", "Weekday", "Bank"]
        ...
        for _, row in top5.iterrows():
            ...
            for sig in signals:
                ...
                items.append(html.Div([
                    html.Span(direction + " ",
                              style={"color": d_color, "fontWeight": "700"}),
            ...
            why_cards.append(html.Div([
                html.Div([
                    ...
                    html.Span(f"  {row['score']:.3f}",
                              style={"color": t_color, "fontWeight": "600",
                                     "fontSize": "0.85rem"}),
                ...
                html.Div(items),
                html.Div(t_desc, style={
                    "fontSize": "0.7rem", "color": C_MUTED, "marginTop": "8px",
        ...
        why_section = html.Div([
            ...
            html.Div("▲ = above the average consumer card (business-like)   "
                     "▼ = below average (also business-like for signals such as evening activity)",
        ...
        return html.Div([
            ...
            html.Div([
                ...
                dash_table.DataTable(
                    ...
                    style_header={"background": C_GRID, "fontWeight": "600",
                                  "color": C_TEXT},
